...
import stableNgFactorySwapABI from "../constants/abis/factory-stable-ng/factory-stable-ng.json" assert { type: 'json' };
...
async function getRecentlyStableNgFactoryPoolId(this: ICurve, swapAddress: string): Promise<string> {
    ...
    for (let i = 1; i <= poolCount; i++) {
        ...
        if (address.toLowerCase() === swapAddress.toLowerCase()) return `factory-stable-ng-${poolCount - i}`
    }

    throw Error("Unknown pool")
}

async function getStableNgFactoryIdsAndSwapAddresses(this: ICurve, fromIdx = 0): Promise<[string[], string[]]> {
    const factoryContract = this.contracts[this.constants.ALIASES.stable_ng_factory].contract;
    ...
    let factories: { id: string, address: string}[] = (await this.multicallProvider.all(calls) as string[]).map(
        (addr, i) => ({ id: `factory-stable-ng-${fromIdx + i}`, address: addr.toLowerCase()})
    );
...
function setStableNgFactorySwapContracts(this: ICurve, factorySwapAddresses: string[]): void {
    factorySwapAddresses.forEach((addr) => {
        this.setContract(addr, stableNgFactorySwapABI);
    });
}

function setStableNgFactoryTokenContracts(this: ICurve, factoryTokenAddresses: string[]): void {
    factoryTokenAddresses.forEach((addr) => {
        this.setContract(addr, ERC20ABI);
    });
}

function setStableNgFactoryGaugeContracts(this: ICurve, factoryGaugeAddresses: string[]): void {
...
function setStableNgFactoryCoinsContracts(this: ICurve, coinAddresses: string[][]): void {
...
function getStableNgFactoryUnderlyingCoinAddresses(this: ICurve, coinAddresses: string[][]): string[][] {
...
export async function getStableNgFactoryPoolData(this: ICurve, fromIdx = 0, swapAddress?: string): Promise<IDict<IPoolData>> {
    const [poolIds, swapAddresses] = swapAddress ?
        [[await getRecentlyStableNgFactoryPoolId.call(this, swapAddress)], [swapAddress.toLowerCase()]]
        : await getStableNgFactoryIdsAndSwapAddresses.call(this, fromIdx);
    if (poolIds.length === 0) return {};

    const [gaugeAddresses, coinAddresses] = await getPoolsData.call(this, swapAddresses);
    setStableNgFactorySwapContracts.call(this, swapAddresses);
    setStableNgFactoryGaugeContracts.call(this, gaugeAddresses);
    setStableNgFactoryCoinsContracts.call(this, coinAddresses);
    const underlyingCoinAddresses = getStableNgFactoryUnderlyingCoinAddresses.call(this, coinAddresses);
    ...
    const STABLE_NG_FACTORY_POOLS_DATA: IDict<IPoolData> = {};
    for (let i = 0; i < poolIds.length; i++) {
        STABLE_NG_FACTORY_POOLS_DATA[poolIds[i]] = {
            name: poolNames[i],
            full_name: poolNames[i],
            symbol: poolSymbols[i],
            reference_asset: "OTHER",
            ...
            swap_abi: stableNgFactorySwapABI,
            gauge_abi: this.chainId === 1 ? factoryGaugeABI : gaugeChildABI,
        };
    }

    return STABLE_NG_FACTORY_POOLS_DATA
}

// TODO DELETE AFTER READY FACTORY
export function DummyFunction(functionName: string) {
    console.log(`${functionName} has not yet been implemented for factory-stable-ng.`)
}