...
export async function _getUsdPricesFromApi(this: Curve): Promise<IDict<number>> {
...
export async function _getCrvApyFromApi(this: Curve): Promise<IDict<[number, number]>> {
    const network = this.constants.NETWORK_NAME;
    const allTypesExtendedPoolData = await _getAllPoolsFromApi(network, this.isLiteChain);
